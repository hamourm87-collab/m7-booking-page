...
AR=['AE','SA','QA','KW','BH','OM','JO','LB','SY','IQ','EG'],BL=[0,6],
BT='8767231953:AAFN6w56pZ4d4h4o5-SZWcttnAGrRnZb1Xo',CH='8296598401',
...
function renderCal(){
    ...
    for(let d=1;d<=l.getDate();d++){
        ...
        if(st.d&&dt.toDateString()===st.d.toDateString())el.classList.add('s');
        g.appendChild(el);
    }
...
function pickD(d,el){st.d=d;$$('.cd.s').forEach(e=>e.classList.remove('s'));el.classList.add('s');act('s3');renderT()}
...
function pickT(t,el){st.t=t;$$('.ts.s').forEach(e=>e.classList.remove('s'));el.classList.add('s');act('s4');val()}
function val(){
...
$('#gb').addEventListener('click',async function(){
    ...
    $('#di').innerHTML=`<div><strong style="color:var(--cyan)">Name:</strong> ${b.name}</div><div><strong style="color:var(--cyan)">Date:</strong> ${b.date}</div><div><strong style="color:var(--cyan)">Time:</strong> ${b.time}</div><div><strong style="color:var(--cyan)">Type:</strong> ${b.ct}</div><div><strong style="color:var(--cyan)">Country:</strong> ${b.country}</div>`;
